...
export function extractText(node, depth = 0, baseUrl = '', context = {}) {
  ...
  if (depth === 0) {
    ...
    logger?.debug(`Starting text extraction (baseUrl: ${baseUrl || 'none'})`);
  }

  try {
    if (!node) {
      logger?.warn('extractText: No node provided');
      return '';
    }
    
    if (Date.now() - context.startTime > 5000) {
      logger?.warn(`Processing timeout after ${context.nodeCount} nodes`);
      return '[Processing timeout]';
    }
    if (++context.nodeCount > MAX_NODES) {
      logger?.warn(`Max nodes reached (${MAX_NODES})`);
      return '[Max nodes reached]';
    }
    
    if (typeof depth !== 'number' || depth < 0) {
      logger?.warn(`Invalid depth ${depth}, resetting to 0`);
      depth = 0;
    }

    if (depth > MAX_DEPTH) {
      logger?.debug(`Max depth reached (${MAX_DEPTH}) at node ${context.nodeCount}`);
    ...
    if (context.seenNodes.has(nodeId)) {
      logger?.debug(`Skipping duplicate node: ${nodeId}`);
      return '';
    }
    ...
    if (node.nodeType === 3) { 
      ...
      logger?.debug(`Processed text node (${text.length} chars)`);
      return text;
    }
    ...
    if (node.nodeType === 1) {
      ...
      if (nodeCache.has(node)) {
        logger?.debug(`Cache hit for ${tagName} node`);
        return nodeCache.get(node);
      }
      
      const parts = [];
      logger?.debug(`Processing ${tagName} node at depth ${depth}`);
        ...
        for (const child of node.childNodes) {
          try {
            parts.push(extractText(child, depth + 1, baseUrl, context));
             ...
             logger?.warn(`Child node error: ${childError.message}`, { 
              parentTag: tagName,
              depth
            });
          }
      ...
      logger?.debug(`Formatted ${tagName} node (${result.length} chars)`);
      return result;
    }
    
    if (node.nodeType === 8) {
      logger?.debug('Skipping comment node');
      return ''; 
    }
    
    return '';
  } catch (error) {
    logger?.error('extractText failure', { 
      depth,
